...
export function listTransactions() {
  ...
  const rows = db.prepare(
    'SELECT id, merchant, date, category, amount, status, icon, payment_method, type, recurring_interval FROM transactions ORDER BY date DESC, created_at DESC'
  ).all();
...
export function createTransaction(tx) {
  ...
  db.prepare(
    `INSERT INTO transactions (id, merchant, date, category, amount, status, icon, payment_method, type, recurring_interval, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    tx.merchant ?? '',
  ...
  const row = db.prepare('SELECT id, merchant, date, category, amount, status, icon, payment_method, type, recurring_interval FROM transactions WHERE id = ?').get(id);
